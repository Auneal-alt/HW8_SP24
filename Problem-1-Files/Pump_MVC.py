...
class Pump_Controller():
    ...
    def ImportFromFile(self, data):
        ...
        :return:
        """
        self.Model.PumpName = data[0].strip()  # JES Missing Code
        units_line = data[2].split()  # "data[1] is the units line" is what was noted but its actually 3rd row?
        L = data[2].split()

        self.Model.FlowUnits = units_line[0].strip()  # JES Missing Code
        self.Model.HeadUnits = units_line[1].strip()  # JES Missing Code
        self.Model.EffData = units_line[2].strip()  # this was missing but I added it because it seemed to be missing
    ...
    def SetData(self, data):
        ...
        # parse new data
        for L in data:
            Cells = L.split()  # JES Missing Code #parse the line into an array of strings
            self.Model.FlowData = np.append(self.Model.FlowData, float(
                Cells[0]))  # JES Missing Code) #remove any spaces and convert string to a float
            self.Model.HeadData = np.append(self.Model.HeadData, float(
                Cells[1]))  # JES Missing Code) #remove any spaces and convert string to a float
            self.Model.EffData = np.append(self.Model.EffData, float(
                Cells[2]))  # JES Missing Code) #remove any spaces and convert string to a float
...
class Pump_View():
    ...
    def DoPlot(self, Model):
        ...
        # Plot head data
        ax1.plot(headx, heady, label='Head', color='black', linestyle='--')
        ax1.scatter(Model.FlowData, Model.HeadData, color='none', edgecolors='black', marker='o', label='Head Points')

        # Create a twin axes for efficiency
        ax2 = ax1.twinx()

        # Plot efficiency data on ax2
        ax2.plot(effx, effy, label='Efficiency', color='black', linestyle=':')
        ax2.scatter(Model.FlowData, Model.EffData, color='none', edgecolors='black', marker='^', label='Efficiency Points')

        # Set labels for axes
        ax1.set_xlabel('X Label')
        ax1.set_ylabel('Head (ft)')
        ax2.set_ylabel('Efficiency (%)')
        # How I added the decimal limit to the Rsquared
        headRSq_formatted = "{:.3f}".format(headRSq)
        effRSq_formatted = "{:.3f}".format(effRSq)

        # Create Legneds
        ax1.legend(['Head ($R^2$=' + headRSq_formatted + ')', 'Head'], loc=6, fontsize=7)
        ax2.legend(['Efficiency ($R^2$=' + effRSq_formatted + ')', 'Efficiency'], loc=1, fontsize=7)
